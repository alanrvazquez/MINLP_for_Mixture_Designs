...
* problem
model COMMON /vec_fim1, vec_fim2, vec_fim3, est_model1, est_model2, est_model3
              mat_fim, symm_fim, eq_l, eq_u1, chol_sta, ing_avail, ord_nw, ord_x,
              const_mixt, lin_iav_1, lin_iav_2, lin_iav_3/;
...
*parameters Dguess(nv, np), yguess(np), Lguess(nth,nth1);
*$gdxin 'in.gdx'
*$load Dguess, yguess, Lguess
*$gdxin
*;
*x.l(nv,np) = Dguess(nv,np);
*y.l(np)       =      yguess(np);

* Initial parameters and bounds
dif_th.lo(nth,np1)      =       -0.1;
dif_th.up(nth,np1)      =       1.1;
*dif_th.l('1',np)        =       x.l('1',np);
*dif_th.l('2',np)        =       x.l('2',np);
*dif_th.l('3',np)        =       x.l('3',np);;
*dif_th.l('4',np)        =       x.l('1',np)*x.l('2',np);
*dif_th.l('5',np)        =       x.l('1',np)*x.l('3',np);
*dif_th.l('6',np)        =       x.l('2',np)*x.l('3',np);

fim_th.lo(nth, nth1)    =       -0.1;
fim_th.up(nth, nth1)    =       1.1;
*fim_th.l(nth,nth1)      =       sum(np, (y.l(np)/npz)*
*                                dif_th.l(nth, np)*dif_th.l(nth1,np));

* initialization for D-optimality
lm.lo(nth,nth1)         =       -1;
lm.lo(nth,nth)          =       1.0e-5;
lm.up(nth,nth1)         =       1;
*lm.l(nth,nth1)          =       Lguess(nth,nth1);
...
*parameters Xinit(nv,np), LforAinit(nth,nth);
*Xinit(nv,np) = x.l(nv,np);
*LforAinit(nth,nth1) = lm.l(nth,nth1);
*execute_unload 'check_initial.gdx' Xinit, LforAinit;
...
*Lsol(nth,nth)        'save L matrix, Cholesky'

* solution for D-optimality
*option limrow = 10000 ; To print all constraints in the model
...
time                =       D_opt.resusd;
*Lsol(nth,nth1)      =       lm.l(nth,nth1);

*execute_unload 'check_optimal.gdx' xsol, nsol, Lsol;
